...
use crate::server::types::{ChannelMessage, NotificationEvent, AdapterState};
...
impl AdapterManager {
    ...
    async fn dispatch_adapter_event(&self) {
        ...
        if adapter.is_some() {
            let state = adapter.unwrap().adapter_state().await.unwrap();
            let powered = state == CentralState::PoweredOn;
            self.send_notification(NotificationEvent::AdapterStateChanged { powered, state: AdapterState::from(state) })
            .await;
        }
    }
    ...
    pub async fn get_adapter(&self) -> Result<Option<Adapter>, Box<dyn std::error::Error>> {
        ...
        // - macos returns CentralState::Unknown if permissions was declined just now. it will throw "PermissionDenied" on any other try.
        let adapter = utils::get_adapter(&self.manager, None).await;
        if adapter.is_some() {
            let adapter_state = adapter.clone().unwrap().adapter_state().await.unwrap();
            info!("Adapter found without loader {:?}", adapter_state);
            if adapter_state != CentralState::Unknown {
        ...
        // once Adapter is found and assigned then it correctly reports it's state change (on/off)
    ...
    pub async fn adapter_loader(&self) {
        ...
        let watcher = tokio::spawn(async move {
            loop {
                ...
                if adapter_found {
                    info!("Adapter found {:?}", adapter_found);
                ...
                // similar to utils::get_adapter but we want to know error message
                let adapter = match manager.adapters().await {
                    Ok(adapters) => {
                        adapters.into_iter().nth(0)
                    },
                    Err(e) => {
                        // macos: CBManagerAuthorization(2) > PermissionDenied
                        if e.to_string() == "Permission denied" {
                            self_clone.send_notification(NotificationEvent::AdapterStateChanged {
                                powered: false,
                                state: AdapterState::PermissionDenied
                            }).await;
                        }
    ...
    pub async fn send_notification(&self, message: NotificationEvent) {
        info!("send_notification: {:?}", message.clone());
        let state = self.adapter_watcher.lock().await;
        let listeners = state.listeners.clone();
        for listener in &listeners {
            if let Err(e) = listener.send(ChannelMessage::Notification(message.clone())) {
    ...
    async fn add_device(&self, device: TrezorDevice) {
        ...
        if let Some(outdated) = outdated {
            ...
            // self.send_notification(NotificationEvent::DeviceRemoved {
            //     id: outdated.get_id(),
            // }).await;
        }
    ...
    async fn prune_devices(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for x in devices {
            ...
            if let Err(adp_per) = utils::get_peripheral_by_id(adapter, x.get_id()).await {
                peripherals.remove(&x.get_id());
                removed.push(x.get_id().to_string());
    
                match x.get_connection_status() {
                    DeviceConnectionStatus::Connected => {
                        disconnected.push(x.get_id().to_string());
                    },
                    _ => {},
                }
            }
        ...
        if disconnected.len() > 0 {
            ...
            for id in removed.iter() {
                self.send_notification(NotificationEvent::DeviceDisconnected{
                    id: id.clone(),
            ...
            while let Some(event) = events.next().await {
                match event {
                    CentralEvent::StateUpdate(state) => {
                        ...
                        self_clone
                            .send_notification(NotificationEvent::AdapterStateChanged { powered, state: AdapterState::from(state) })
                            .await;
                    }
                    CentralEvent::DeviceDiscovered(id) => {
                        ...
                        if evt.is_some() {
                            ...
                            self_clone.prune_devices().await;
                    ...
                    CentralEvent::DeviceUpdated(id) => {
                        ...
                        if device.is_some() {
                            ...
                            if emit_update {
                                ...
                                let devices = self_clone.get_devices().await;
                                self_clone
                                    .send_notification(NotificationEvent::DeviceUpdated { id, devices })
                                    .await;
                            }
                        }
                    }
    ...
    // TODO: rename to remove listener
