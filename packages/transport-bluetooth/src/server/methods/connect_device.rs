...
async fn send_notification(
    ...
    manager
        .send_notification(NotificationEvent::DeviceConnectionStatus(device))
        .await
}
...
async fn connect_device_target(
    ...
    if device.is_paired() {
        ...
        return connect_device_common(id, adapter, manager).await;
    }
    ...
    manager
        .send_notification(NotificationEvent::DeviceSettingsUi)
        .await;
    ...
    tokio::select! {
        response = props_task.as_mut().unwrap() => {
            ...
            if let Some(err) = response.unwrap() {
                send_notification(manager.clone(), device.clone(), DeviceConnectionStatus::PairingError{ error: err.to_string() }).await;
                return Err(err)?;
            }
        },
...
async fn connect_device_target(
    ...
    if !pairing.IsPaired()? {
        ...
        let bt_device = manager.get_device(id.clone()).await.unwrap();
...
pub async fn connect_device(id: String, manager: AdapterManager) -> MethodResult {
    let adapter = manager.get_adapter().await?;
    if !(utils::is_adapter_powered(adapter.clone()).await) {
        return Err("AdapterDisabled")?;
    }

    return connect_device_target(id, adapter, manager).await;
}
...
async fn connect_device_common(
    ...
    if result != "Success" {
        bt_device.set_connection_status(DeviceConnectionStatus::PairingError{ error: result });
