...
async fn handle_ws_connection(
    ...
    while let Some(msg) = ws_read.next().await {
        ...
        let ws_write_method = ws_write_method.clone();
    ...
    if let Err(err) = sender.send(ChannelMessage::Abort(AbortProcess::Disconnect)) {
        info!("Closing websocket connection error {}", err);
    }
...
async fn handle_http_request(
    ...
    if hyper_tungstenite::is_upgrade_request(&req) {
        ...
        tokio::spawn(async move { handle_ws_connection(peer, websocket, manager).await });
        Ok(response)
    } else {
