...
export class BluetoothApi extends AbstractApi {
    ...
    async init() {
        ...
        // try reconnect
        api.on('disconnected', () => api.connect());
    ...
    async enumerate() {
        const devices = await this.api.send('enumerate').catch(() => []);

        return Promise.resolve(this.success(this.devicesToDescriptors(devices)));
    }
    ...
    async write(path: string, buffer: Buffer) {
        console.warn('Device write1', buffer);
        const result = await this.api.send('write', [path, Array.from(buffer)]);
        console.warn('Device write2', result);
